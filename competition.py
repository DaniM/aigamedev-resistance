...
class CompetitionRunner(object):
    ...
    def main(self):
        ...
        self.competitorsCombinations = map(lambda x: x, itertools.combinations(self.competitors, 5))
        self.roleCombinations = map(lambda x: [True if y in x else False for y in range(5)], itertools.combinations(range(5),2))

        i = 0
#        for j in range(int(math.ceil(float(len(self.competitorsCombinations)*len(self.roleCombinations))/self.rounds))):
        for j in range(1,self.rounds+1):
            random.shuffle(self.competitorsCombinations)
            for c in self.competitorsCombinations:
                random.shuffle(self.roleCombinations)
                for r in self.roleCombinations:
                    self.play(CompetitionRound, c, r)
                    i += 1
...
if __name__ == '__main__':
    ...
    try:
            runner.main()
##        cProfile.run('runner.main()', 'resistance_prof')
##        import pstats
##        p = pstats.Stats('resistance_prof')
