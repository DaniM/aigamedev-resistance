#-------------------------------------------------------------------------------
# Name:        mybots
# Purpose:     aigamedev The Resistance Competition
#
            ...
            if sorted_by_trust[0][1] <= 0.5:
                ...
                for j in range(5):
                    if owner.memory.selections.value[0][j] > 0:
                        previous.append(game.players[j])
                    else:
                        players.add(game.players[j])

                if owner in previous:
                    previous.remove(owner)
                    team = random.sample(previous,  1) + random.sample(players,  2)
                else:
...
class OnMissionCompletedSpySabotagedBehavior(ResistanceBaseBehavior):
    """One spy on the team"""
    def process(self, game, owner, phase):
        if owner.memory.lastSabotage > 0:
            for member in owner.memory.currentTeam:
                owner.entries.addTrust(member, -2/len(owner.memory.currentTeam))
                #get the selections
                owner.memory.selections.value[game.turn - 1][member.index -1] = 1
            owner.entries.addTrust(owner.memory.currentLeader, -0.5)

            resistance = set(owner.memory.currentTeam) - owner.memory.spies
            spy = set(owner.memory.currentTeam) - resistance

            resistance = list(resistance)
            spy = list(spy)[0]

            #if the team is a group of three and everyone knows that the other two are not spies
            if len(resistance) == 2 and owner.entries.entries[resistance[0]] > 2 and owner.entries.entries[resistance[1]] > 2:
                owner.entries.addTrust(spy, -4)
    ...
    def process(self, game, owner, phase):
        if owner.memory.lastSabotage == 0:
            ...
            if not (owner.memory.currentLeader in owner.memory.currentTeam) and owner.memory.currentLeader!= owner:
                owner.entries.addTrust(owner.memory.currentLeader, 2)
...
class OnMissionCompletedSpyBehavior(ResistanceCompositeBaseBehavior):
    ...
    def __init__(self, game, owner, priority = 0, children=[]):
        ...
        self.children = [#OnMissionCompletedResistanceFailBehavior(game, owner, 1),
                        OnMissionCompletedSpySabotagedBehavior(game, owner, 1),
                        OnMissionCompletedResistanceFailBCBehavior(game, owner, 0),
                        OnMissionCompletedResistanceBCBehavior(game, owner, 0),]
        self.children.sort()
